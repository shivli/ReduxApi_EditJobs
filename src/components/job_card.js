                            ...
                            return (
                                <div className="container card">
                                    ...
                                    <div className='col-sm-8'>
                                        ...
                                        <div >{info.city}</div>
                                        {(localStorage.getItem('user_type') === '2' || localStorage.getItem('user_type') === null) ? (applied_ids.find((ele) => { return ele === info._id }) ? <button onClick={this.applied} className="btn btn-primary" type="button">Applied</button> : <button id={info._id} onClick={(e) => this.apply(info, e)} className="btn btn-success" type="button">Apply</button>) : <button id={info._id} onClick={(e) => this.handleClick(info, e)} className="btn btn-success" type="button">Edit</button>}
                                        {/* <div>{localStorage.getItem('user_type') === '3' && <button onClick={(e) => this.handleClick(info, e)}>
                                            Edit
                                            </button>}</div>
                                        <div>{localStorage.getItem('user_type') === '2' && <button onClick={(e) => this.applyClick(info, e)}>
                                            Apply
                                            </button>}</div> */}
